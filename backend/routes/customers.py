...
from validators.cif_validator import validate_cif
...
def create_customer():
    ...
    if len(tax_identifier) < 5:
        ...
        return jsonify({"error": "Invalid tax identifier."}), 400
    ...
    if data["country"].lower() == "spain":
        ...
        if tax_type == "DNI":
            ...
            valid = validate_dni(tax_identifier)

        elif tax_type == "NIF":

            valid = validate_nif(tax_identifier)

        elif tax_type == "CIF":

            valid = validate_cif(tax_identifier)

        else:

            return jsonify({"error": "Invalid tax identifier type."}), 400

        if not valid:

            return jsonify({"error": f"Invalid Spanish {tax_type}."}), 400
    ...
    if existing_customer:
        ...
        return (
            jsonify({"error": "A customer with this tax identifier already exists."}),
            409,
        )

    customer = Customer(
        company_name=data["company_name"].strip(),
        tax_type=data["tax_type"].strip().upper(),
        tax_identifier=tax_identifier.upper(),
...
def update_customer(customer_id):
    ...
    customer.tax_type = data["tax_type"].strip().upper()
